...
class User():
    def __init__(
            self, id: int, name: str, bot: bool,
            mute: bool = False,
            threat: float = 0,
            messages: list = []
            ) -> None:
        self.id = id              # ユーザーID
        self.name = name          # ユーザー名を取得
        self.bot = bot            # BOTユーザーかどうか
        self.mute = mute          # ミュートの処理を送信済みか（APIでミュートを確認しているわけではない。）
        self.threat = threat      # ユーザーの脅威
...
class Word():
    ...
    def __add__(self, other):
        ...
        if (self.end + 1 - other.top) != 0:
            raise ValueError(
                ("単語が連続した順番ではありません。"
                 f"[({self.word,self.top,self.end}"
                 f"{other.word,other.top,other.end})]")
                )
        return Word(
            id=None, word=self.word+other.word,
            top=self.top, end=other.end
            )
...
async def on_message(message: Message, tmplist):
    if message.user.bot:
        return print("処理はパスされました。理由:BOT")
    # 一時保存場所にギルドデータがない場合は追加する。
    if message.guild.id not in [guild.id for guild in tmplist]:
        tmplist.append(TmpGuild(id=message.guild.id))
